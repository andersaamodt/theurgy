...
- macOS generated adapters must dispatch declared runtime manifest commands directly and must not stage the `theurgy-runtime` wrapper into professional app bundles. Linux desktop generated runtimes still expose `requestCommand` as `["theurgy-runtime", "run-request"]` and `requestCommandManifest` equal to `runtimeManifest` for the current local typed request dispatcher; mobile generated runtimes must omit local request commands and stay on external JSON ABI handoff.
- Mobile JSON ABI handoffs must carry Theurgy-owned request schema IDs for state, status, subscribe-status, action, operation-status, and operation-history requests, so platform hosts are not coupled to undocumented envelope conventions. Generated mobile hosts should expose that boundary as a native request broker/adapter interface, not as local process command execution.
...
- Runtime manifest command execution resolves bare executable names relative to the manifest directory, then adjacent `bin/` and `libexec/`, before falling back to `PATH`; installed adapters can therefore bundle their staged runtime beside the manifest without depending on a source checkout.
