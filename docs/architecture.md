...
- Desktop generated runtimes must expose `requestCommand` as `["theurgy-runtime", "run-request"]` and `requestCommandManifest` equal to `runtimeManifest` for the local typed request dispatcher; mobile generated runtimes must omit local request commands and stay on external JSON ABI handoff.
