...
- Install Playwright-managed browser binaries through `spells/install-theurgy-browser-proof-runtime` so desktop browser target checks stay repeatable and quarantined outside app repositories; if the Playwright package is absent, the spell installs it under Theurgy's user-local state root before installing browsers, and it must not use Homebrew for this dependency.
