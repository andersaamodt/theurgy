...
- Use `spells/install-theurgy-browser-proof-runtime` for Playwright-managed browser binaries; when the Playwright package is absent, the spell installs it under Theurgy's user-local state root, and browser proof dependencies must not be installed through Homebrew or ad hoc project-local caches.
