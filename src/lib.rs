pub mod product_runtime {
    ...
    #[derive(Clone, Debug, Eq, PartialEq)]
    pub struct MobileScreenContract {
        pub id: String,
        pub title: String,
        pub node_id: String,
    ...
    pub fn validate_runtime_state_request_value(
        value: &Value,
    ) -> ContractResult<RuntimeStateRequest> {
        expect_value_string(value, "schema", RUNTIME_STATE_REQUEST_SCHEMA)?;
        expect_value_string(value, "protocol", RUNTIME_ACTION_PROTOCOL)?;
        let app_id = value_string(value, "app")
            .filter(|id| valid_runtime_request_app_id(id))
            .ok_or_else(|| {
                ContractError::new("runtime state request app must be a lowercase slug")
            })?;
        expect_value_string(value, "kind", "state")?;
        Ok(RuntimeStateRequest { app_id })
    }

    pub fn validate_runtime_status_request_value(
        value: &Value,
    ) -> ContractResult<RuntimeStatusRequest> {
        expect_value_string(value, "schema", RUNTIME_STATUS_REQUEST_SCHEMA)?;
        expect_value_string(value, "protocol", RUNTIME_ACTION_PROTOCOL)?;
        let app_id = value_string(value, "app")
            .filter(|id| valid_runtime_request_app_id(id))
            .ok_or_else(|| {
                ContractError::new("runtime status request app must be a lowercase slug")
            })?;
    ...
    pub fn validate_generated_runtime_value(value: &Value) -> ContractResult<GeneratedRuntime> {
        ...
        let runtime_state_request_schema = expect_and_return_value_string(
            value,
            "runtimeStateRequestSchema",
            RUNTIME_STATE_REQUEST_SCHEMA,
        )?;
        let runtime_status_request_schema = expect_and_return_value_string(
            value,
        ...
        if surface_screen_contracts.len() != surface_screens.len() {
            return Err(ContractError::new(
                "generated runtime surfaceScreenContracts must match surfaceScreens length",
            ));
        }
        ...
        if surface_screen_contract_ids != surface_screens {
            return Err(ContractError::new(
                "generated runtime surfaceScreenContracts order must match surfaceScreens",
            ));
        }
        ...
        if matches!(target.as_str(), "ios" | "android") && surface_screens.is_empty() {
            return Err(ContractError::new(
                "generated runtime mobile targets require surfaceScreens",
            ));
        }
    ...
    pub fn generated_runtime_metadata(
        ...
        object.insert(
            "runtimeStateRequestSchema".to_string(),
            Value::String(RUNTIME_STATE_REQUEST_SCHEMA.to_string()),
        );
        object.insert(
        ...
        object.insert(
            "surfaceScreenContracts".to_string(),
            mobile_screen_contracts_value(&surface.mobile_screens),
        );
    ...
    pub fn macos_adapter_files(
        ...
        vec![
            GeneratedNativeFile {
                ...
                contents: format!(
                    "// Generated by theurgy-runtime compile-native.\n// swift-tools-version: 6.0\nimport PackageDescription\n\nlet package = Package(name: \"{}\", platforms: [.macOS(.v13)], products: [.executable(name: \"{}\", targets: [\"App\"])], targets: [.executableTarget(name: \"App\", path: \"Sources/App\", resources: [.copy(\"Resources\")])])\n",
    ...
    pub fn linux_adapter_files(
        ...
        vec![
            GeneratedNativeFile {
                ...
                contents: format!(
                    "project('{}', 'c', version: '0.1.0')\ngtk = dependency('gtk4')\njson_glib = dependency('json-glib-1.0')\nexecutable('{}', 'src/main.c', dependencies: [gtk, json_glib], install: true)\ninstall_data('theurgy-runtime.json', 'theurgy-surface.json', 'runtime.manifest.json', install_dir: get_option('datadir') / 'theurgy' / '{}')\n",
    ...
    pub fn ios_adapter_files(
        ...
        vec![
            GeneratedNativeFile {
                path: "Package.swift".to_string(),
                contents: format!(
                    "// Generated by theurgy-runtime compile-native.\n// swift-tools-version: 6.0\nimport PackageDescription\n\nlet package = Package(name: \"{}-ios\", platforms: [.iOS(.v16), .macOS(.v13)], products: [.executable(name: \"{}IOS\", targets: [\"Host\"])], targets: [.executableTarget(name: \"Host\", path: \"Host\", resources: [.copy(\"Resources\")])])\n",
                    product.app_id, product.app_id
                ),
            },
            GeneratedNativeFile {
    ...
    fn mobile_screen_contracts_value(screens: &[MobileScreenContract]) -> Value {
        Value::Array(
            screens
                .iter()
                .map(|screen| {
                    let mut object = serde_json::Map::new();
                    object.insert("id".to_string(), Value::String(screen.id.clone()));
                    object.insert("title".to_string(), Value::String(screen.title.clone()));
                    object.insert("nodeId".to_string(), Value::String(screen.node_id.clone()));
                    object.insert(
                        "nodeType".to_string(),
                        Value::String(screen.node_type.clone()),
                    );
                    if let Some(role) = &screen.role {
...
func surfaceScreenSummaries(_ json: String) -> [String] {
...
struct RuntimeContract {
  ...
  var surfaceScreens: [String] { surfaceScreenSummaries(surfaceMetadata) }
  let protocolName = "__PROTOCOL__"
  let actionContracts = __ACTION_CONTRACTS__
  let screenContracts = __SCREEN_CONTRACTS__
  ...
  func stateEnvelope() -> String {
    let envelope: [String: Any] = [
      "schema": runtimeStateRequestSchema,
      "protocol": protocolName,
      "app": runtimeApp,
      "kind": "state"
    ]
  ...
  func statusEnvelope() -> String {
    let envelope: [String: Any] = [
      "schema": runtimeStatusRequestSchema,
      "protocol": protocolName,
      "app": runtimeApp,
      "kind": "status"
    ]
...
struct MobileRuntimeHandoff {
  let kind: String
  let requestJson: String
}

protocol MobileRuntimeRequestBroker {
  func submit(kind: String, requestJson: String) -> MobileRuntimeHandoff
}

struct ExternalJsonAbiPreviewBroker: MobileRuntimeRequestBroker {
  func submit(kind: String, requestJson: String) -> MobileRuntimeHandoff {
    MobileRuntimeHandoff(kind: kind, requestJson: requestJson)
  }
}

struct RuntimeContractView: View {
  let contract = RuntimeContract()
  let broker = ExternalJsonAbiPreviewBroker()

  var body: some View {
    TabView {
      ForEach(contract.screenContracts, id: \.id) { screen in
        MobileSurfaceScreenView(contract: contract, broker: broker, screen: screen)
          .tabItem {
            Text(screen.title)
          }
      }
...
struct MobileSurfaceScreenView: View {
  let contract: RuntimeContract
  let broker: any MobileRuntimeRequestBroker
  let screen: MobileScreenContract
  @State private var lastHandoff = "No request submitted"

  var body: some View {
    NavigationStack {
      List {
        Section("Request Handoff") {
          Text("Transport: \(contract.runtimeTransport)")
          Text(lastHandoff)
          Button("Hydrate State") {
            let handoff = broker.submit(kind: "state", requestJson: contract.stateEnvelope())
            lastHandoff = "\(handoff.kind): \(handoff.requestJson)"
          }
          Button("Refresh Status") {
            let handoff = broker.submit(kind: "status", requestJson: contract.statusEnvelope())
            lastHandoff = "\(handoff.kind): \(handoff.requestJson)"
          }
          Button("Subscribe Status") {
        ...
        Section("Actions") {
          ForEach(contract.actionContracts(for: screen), id: \.id) { action in
            VStack(alignment: .leading) {
              ...
              Button("Submit \(action.label)") {
                let handoff = broker.submit(kind: "action", requestJson: contract.actionEnvelope(for: action, params: contract.defaultParams(for: action)))
                lastHandoff = "\(handoff.kind): \(handoff.requestJson)"
              }
            }
          }
...
"#;
        template
            ...
            .replace(
                "__SCREEN_CONTRACTS__",
                &swift_mobile_screen_contracts_literal(&surface.mobile_screens),
            )
...
public final class MainActivity extends Activity {
  ...
  private static final MobileScreenContract[] SCREEN_CONTRACTS = __SCREEN_CONTRACTS__;

  private interface MobileRuntimeRequestBroker {
    String submit(String kind, String requestJson);
  }

  private static final class ExternalJsonAbiPreviewBroker implements MobileRuntimeRequestBroker {
    @Override public String submit(String kind, String requestJson) {
      return kind + ": " + requestJson;
    }
  }
  ...
  private static final class MobileScreenContract {
    final String id;
    final String title;
    final String nodeId;
  ...
  private static String stateEnvelope(String app, String requestSchema) {
    try {
      JSONObject envelope = new JSONObject();
      envelope.put("schema", requestSchema);
      envelope.put("protocol", PROTOCOL);
      envelope.put("app", app);
      envelope.put("kind", "state");
      return envelope.toString();
    } catch (JSONException error) {
      return "{}";
    }
  ...
  private static String statusEnvelope(String app, String requestSchema) {
    try {
      JSONObject envelope = new JSONObject();
      envelope.put("schema", requestSchema);
      envelope.put("protocol", PROTOCOL);
      envelope.put("app", app);
      envelope.put("kind", "status");
      return envelope.toString();
    } catch (JSONException error) {
      return "{}";
    }
  ...
  @Override public void onCreate(Bundle state) {
    ...
    MobileRuntimeRequestBroker broker = new ExternalJsonAbiPreviewBroker();
    ...
    for (MobileScreenContract screen : SCREEN_CONTRACTS) {
      ...
      button.setOnClickListener((View clicked) -> view.setText(renderScreenText(screen, runtimeMetadata, surfaceMetadata, broker, runtimeApp, runtimeStateRequestSchema, runtimeStatusRequestSchema, runtimeSubscribeStatusRequestSchema, operationStatusRequestSchema, operationHistoryRequestSchema)));
      tabs.addView(button);
    }
    root.addView(tabs);
    ...
    view.setText(renderScreenText(initialScreen, runtimeMetadata, surfaceMetadata, broker, runtimeApp, runtimeStateRequestSchema, runtimeStatusRequestSchema, runtimeSubscribeStatusRequestSchema, operationStatusRequestSchema, operationHistoryRequestSchema));
    setContentView(root);
  }

  private static String renderScreenText(MobileScreenContract activeScreen, String runtimeMetadata, String surfaceMetadata, MobileRuntimeRequestBroker broker, String runtimeApp, String runtimeStateRequestSchema, String runtimeStatusRequestSchema, String runtimeSubscribeStatusRequestSchema, String operationStatusRequestSchema, String operationHistoryRequestSchema) {
    StringBuilder text = new StringBuilder();
    text.append(activeScreen.title).append("\nScreen: ").append(activeScreen.id)
      ...
      .append("\nRequest handoff transport: ").append(jsonString(runtimeMetadata, "adapterRuntimeTransport"))
...
__MOBILE_WORKFLOW_TEXT__
      .append("\nState handoff: ").append(broker.submit("state", stateEnvelope(runtimeApp, runtimeStateRequestSchema)))
      .append("\nStatus handoff: ").append(broker.submit("status", statusEnvelope(runtimeApp, runtimeStatusRequestSchema)))
      .append("\nSubscribe handoff: ").append(broker.submit("subscribe-status", subscribeStatusEnvelope(runtimeApp, runtimeSubscribeStatusRequestSchema)))
      .append("\nOperation status handoff: ").append(broker.submit("operation-status", operationStatusEnvelope(runtimeApp, operationStatusRequestSchema, "default")))
      .append("\nHistory handoff: ").append(broker.submit("operation-history", operationHistoryEnvelope(runtimeApp, operationHistoryRequestSchema, "default", 20)))
      .append("\nActions:");
    for (ProductActionContract action : ACTION_CONTRACTS) {
      ...
      text.append("\n").append(action.label).append(" [").append(action.effect).append("] ")
        .append("\n  handoff: ").append(broker.submit("action", actionEnvelope(runtimeApp, runtimeActionRequestSchema, action, params)));
    }
    return text.toString();
  }
...
"#;
        template
            ...
            .replace(
                "__SCREEN_CONTRACTS__",
                &java_mobile_screen_contracts_literal(&surface.mobile_screens),
            )
...
"#;
    ...
    fn swift_mobile_screen_contracts_literal(screens: &[MobileScreenContract]) -> String {
        let items = screens
            .iter()
            .map(|screen| {
                let role = screen
    ...
    fn java_mobile_screen_contracts_literal(screens: &[MobileScreenContract]) -> String {
        let items = screens
            .iter()
            .map(|screen| {
                let role = screen
            ...
            .collect::<Vec<_>>()
            .join(", ");
        format!("new MobileScreenContract[] {{{items}}}")
    }
    ...
    fn validate_generated_mobile_screen_contract(
        ...
        let id = value_string(contract, "id")
            .filter(|id| valid_action_id(id))
            .ok_or_else(|| {
                ContractError::new("generated runtime mobile screen contract id invalid")
            })?;
        value_string(contract, "title")
            .filter(|title| !title.is_empty())
            .ok_or_else(|| {
                ContractError::new("generated runtime mobile screen contract title required")
            })?;
        value_string(contract, "nodeId")
            .filter(|node_id| !node_id.is_empty())
            .ok_or_else(|| {
                ContractError::new("generated runtime mobile screen contract nodeId required")
            })?;
        value_string(contract, "nodeType")
            .filter(|node_type| !node_type.is_empty())
            .ok_or_else(|| {
                ContractError::new("generated runtime mobile screen contract nodeType required")
            })?;
        optional_nonempty_string(
            contract,
            "role",
            "generated runtime mobile screen contract role",
        )?;
        value_string_array(contract, "roles").map_err(|_| {
            ContractError::new(
    ...
    fn validate_mobile_screen_contract(screen: &Value) -> ContractResult<MobileScreenContract> {
        let id = value_string(screen, "id")
            .filter(|id| valid_action_id(id))
            .ok_or_else(|| ContractError::new("mobile surface screen.id required"))?;
        let title = value_string(screen, "title")
            .filter(|title| !title.is_empty())
            .ok_or_else(|| ContractError::new("mobile surface screen.title required"))?;
        let node = value_object(screen, "node")?;
        let node_id = value_string(node, "id")
            .filter(|node_id| !node_id.is_empty())
            .ok_or_else(|| ContractError::new("mobile surface screen node.id required"))?;
        let node_type = value_string(node, "type")
            .filter(|node_type| !node_type.is_empty())
            .ok_or_else(|| ContractError::new("mobile surface screen node.type required"))?;
        let role = value_string(node, "role").filter(|role| !role.is_empty());
        let mut roles = Vec::new();
        collect_surface_roles(node, &mut roles);
        roles.sort();
        roles.dedup();
...
"#,
  ...
  "surfaceScreenContracts": []
        ...
        assert_eq!(summary.surface_screen_contracts, 0);
    ...
    fn product_runtime_builds_generated_runtime_metadata() {
        ...
        assert!(files.iter().any(|file| file.path == "Package.swift"
            ...
            && file.contents.contains("resources: [.copy(\"Resources\")]")));
        ...
        assert!(meson.contains(
            "install_data('theurgy-runtime.json', 'theurgy-surface.json', 'runtime.manifest.json'"
        ));
        ...
        assert!(files.iter().any(|file| file.path == "Package.swift"
            && file
                .contents
                .contains("platforms: [.iOS(.v16), .macOS(.v13)]")));
        assert!(files.iter().any(|file| file.path == "Host/App.swift"
            ...
            && file
                .contents
                .contains("protocol MobileRuntimeRequestBroker")
            && file
                .contents
                .contains("struct ExternalJsonAbiPreviewBroker: MobileRuntimeRequestBroker")
            && file.contents.contains("Section(\"Request Handoff\")")
            && file.contents.contains("Button(\"Hydrate State\")")
            && file.contents.contains(
                "broker.submit(kind: \"state\", requestJson: contract.stateEnvelope())"
            )
            && file
                .contents
                .contains("broker.submit(kind: \"action\", requestJson: contract.actionEnvelope")
            && !file
                .contents
                .contains("func command(for action: ProductActionContract")
        ...
        assert!(files.iter().any(|file| file.path
            ...
            && file
                .contents
                .contains("private interface MobileRuntimeRequestBroker")
            && file.contents.contains(
                "private static final class ExternalJsonAbiPreviewBroker implements MobileRuntimeRequestBroker"
            )
            && file
            ...
            && file.contents.contains("Request handoff transport: ")
            && file.contents.contains("State handoff: ")
            && file
                .contents
                .contains("broker.submit(\"state\", stateEnvelope")
            && file
                .contents
                .contains("broker.submit(\"action\", actionEnvelope")
            && !file.contents.contains("private static String[] commandFor")
            && file
                .contents
