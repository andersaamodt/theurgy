...
fn compile_native_with_contract(
    ...
    match target {
        ...
        "ios" => compile_ios(summary, runtime, out_dir),
        "android" => compile_android(summary, runtime, out_dir),
...
fn macos_adapter_source(summary: &ProductSummary, runtime: &RuntimeContract) -> String {
    let default_action_id = summary.action_ids.first().cloned().unwrap_or_default();
...
fn compile_ios(summary: &ProductSummary, runtime: &RuntimeContract, out_dir: &Path) -> Result<()> {
    fs::create_dir_all(out_dir.join("Host"))?;
    write_or_replace(
        &out_dir.join("Host/App.swift"),
        &ios_adapter_source(summary, runtime),
    )
}
...
fn compile_android(
    ...
    write_or_replace(
        ...
        &android_adapter_source(summary, runtime),
    )
}

fn ios_adapter_source(summary: &ProductSummary, runtime: &RuntimeContract) -> String {
    let template = r#"// Generated by theurgy-runtime compile-native.
// Runtime: theurgy-runtime.json
// Surface: theurgy-surface.json
import SwiftUI
